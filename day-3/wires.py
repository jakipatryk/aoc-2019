    ...
    def __init__(self, path):
        ...
        for cmd in path:
            ...
            if type_of_cmd == 'L':
                ...
                self.horizontal.append((new_position, current_position))
            elif type_of_cmd == 'R':
                new_position = (
                    current_position[0] + steps, current_position[1])
                self.horizontal.append((current_position, new_position))
            elif type_of_cmd == 'D':
                new_position = (current_position[0], current_position[1]-steps)
                self.vertical.append((new_position, current_position))
            elif type_of_cmd == 'U':
                new_position = (current_position[0], current_position[1]+steps)
                self.vertical.append((current_position, new_position))
        ...
        for segV in s2.vertical:
            if(segH[0][0] <= segV[0][0]
               ...
               and segH[0][1] <= segV[1][1]):
                intersections.append((segV[0][0], segH[0][1]))
...
print(find_min_distance(intersections))
