class Segments:
    def __init__(self, path):
        ...
        for cmd in path:
            ...
            if type_of_cmd == 'L':
                new_position = (
                    current_position[0] - steps, current_position[1])
                self.horizontal.append(
                    (new_position, current_position, -sum_of_steps))
            elif type_of_cmd == 'R':
                new_position = (
                    current_position[0] + steps, current_position[1])
                self.horizontal.append(
                    (current_position, new_position, sum_of_steps))
            elif type_of_cmd == 'D':
                new_position = (current_position[0], current_position[1]-steps)
...
intersections = find_intersections(s1, s2)
print(find_min_sum_distance(intersections))
